...
bool ServerConfig::saveLayout(const QString& configFileName) const
{
    ...
    for (const Screen& screen : screens()) {
        ...
        out << "    {\n"
            ...
            << "      \"host\": \"" << hostName << "\",\n"
            << "      \"name\": \"" << screen.name() << "\",\n"
            << "      \"kind\": \"" << (screen.isBluetooth() ? "bluetooth" : "screen") << "\",\n"
            << "      \"bridgeAddress\": \"" << screen.bluetoothBridgeAddress() << "\",\n"
            << "      \"bridgePort\": " << screen.bluetoothBridgePort() << ",\n"
            << "      \"x\": " << screen.position().x() << ",\n"
            << "      \"y\": " << screen.position().y() << ",\n"
            << "      \"width\": " << size.width() << ",\n"
            << "      \"height\": " << size.height();
...
QTextStream& operator<<(QTextStream& outStream, const ServerConfig& config)
{
    ...
    // Bluetooth HID targets cannot report their real pointer position, so
    // they are entered and left explicitly instead of through screen edges.
    // Keep these shortcuts in every temporary config generated by the GUI.
    QString bluetoothScreenName;
    for (const Screen& screen : config.screens()) {
        if (!screen.isNull() && screen.isBluetooth()) {
            bluetoothScreenName = screen.name();
            break;
        }
    }
    if (!bluetoothScreenName.isEmpty() && !config.m_ServerName.isEmpty()) {
        outStream << "\tkeystroke(Control+Alt+b) = switchToScreen("
                  << bluetoothScreenName << ")" << endl;
        outStream << "\tkeystroke(Control+Alt+p) = switchToScreen("
                  << config.m_ServerName << ")" << endl;
    }
